...
function OnboardingWizard({ onComplete }: { onComplete: (data: any) => void }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative z-0 p-6 bg-white overflow-hidden">
      <div 
        className="absolute inset-0 z-[-1] opacity-40 pointer-events-none bg-cover bg-center" 
        style={{ backgroundImage: "url('https://images.unsplash.com/photo-1490818387583-1b5ba22111d5?auto=format&fit=crop&w=2000&q=80')" }} 
      />
      ...
      <div className="w-full max-w-2xl">
        <div className="mb-10 text-center">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-white rounded-[2rem] shadow-sm border border-slate-100 text-primary-600 mb-6">
            <Activity size={36} strokeWidth={2.5} />
          </div>
          <h1 className="text-4xl font-black mb-2 tracking-tight text-slate-800">Welcome to FitAI</h1>
          <p className="text-slate-500 font-medium text-lg">Let's personalize your fitness journey.</p>
        </div>

        <div className="glass-panel p-10 relative overflow-hidden bg-white border border-slate-100 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)]">
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.3 }} className="space-y-6 mt-4 text-center">
                <h2 className="text-3xl font-black mb-8 text-slate-800">What's your name?</h2>
                <div className="max-w-xs mx-auto">
                  <input autoFocus value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} onKeyDown={e => e.key === 'Enter' && formData.name && nextStep()} placeholder="e.g. Alex" className="w-full glass-input bg-slate-50 shadow-inner text-center text-2xl font-bold py-6" />
                </div>
                <button disabled={!formData.name} onClick={nextStep} className="w-full max-w-xs mx-auto glass-button py-5 mt-8 disabled:opacity-50 disabled:cursor-not-allowed">Continue</button>
              </motion.div>
            )}

            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.3 }} className="space-y-6 mt-4">
                <h2 className="text-3xl font-black mb-8 text-slate-800 text-center">Your Body Metrics</h2>
                <div className="grid grid-cols-2 gap-6 max-w-md mx-auto">
                  <div>
                    <label className="text-xs font-bold text-slate-400 tracking-widest uppercase mb-2 block text-center">Weight (kg)</label>
                    <input type="number" value={formData.weight} onChange={e => setFormData({...formData, weight: e.target.value})} className="w-full glass-input bg-slate-50 text-center text-xl font-bold" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-400 tracking-widest uppercase mb-2 block text-center">Height (cm)</label>
                    <input type="number" value={formData.height} onChange={e => setFormData({...formData, height: e.target.value})} className="w-full glass-input bg-slate-50 text-center text-xl font-bold" />
                  </div>
                  <div className="col-span-2">
                    <label className="text-xs font-bold text-slate-400 tracking-widest uppercase mb-2 block text-center">Age</label>
                    <input type="number" value={formData.age} onChange={e => setFormData({...formData, age: e.target.value})} className="w-full glass-input bg-slate-50 text-center text-xl font-bold" />
                  </div>
                </div>
                <div className="flex justify-center pt-8 flex-col items-center gap-4">
                  ...
                  <button disabled={!formData.weight || !formData.height || !formData.age} onClick={() => {
                    ...
                    setError('');
                    finish();
                  }} className="w-full max-w-md glass-button py-5 disabled:opacity-50 disabled:cursor-not-allowed">Generate My Plan</button>
                </div>
              </motion.div>
            )}
