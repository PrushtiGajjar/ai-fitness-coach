    ...
    <div className="min-h-[100dvh] flex flex-col items-center justify-center py-8 px-4 bg-white overflow-y-auto relative z-0">
      ...
      <div className="w-full max-w-sm my-auto">
        <div className="mb-4 lg:mb-5 text-center">
          <div className="inline-flex items-center justify-center w-12 h-12 lg:w-14 lg:h-14 bg-white rounded-xl lg:rounded-2xl shadow-sm border border-slate-100 text-primary-600 mb-3 lg:mb-4">
            <Activity size={24} className="lg:w-6 lg:h-6" strokeWidth={2.5} />
          </div>
          <h1 className="text-2xl lg:text-2xl font-black mb-1 lg:mb-2 tracking-tight text-slate-800">Welcome to FitAI</h1>
          <p className="text-slate-500 font-medium text-sm lg:text-sm">Let's personalize your fitness journey.</p>
        </div>

        <div className="glass-panel p-4 lg:p-5 relative overflow-hidden bg-white border border-slate-100 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] rounded-3xl">
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.3 }} className="space-y-4 lg:space-y-4 mt-2 lg:mt-3 text-center">
                <h2 className="text-xl lg:text-xl font-black mb-3 lg:mb-4 text-slate-800">What's your name?</h2>
                <div className="max-w-xs mx-auto">
                  <input autoFocus value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} onKeyDown={e => e.key === 'Enter' && formData.name && nextStep()} placeholder="e.g. Alex" className="w-full glass-input bg-slate-50 shadow-inner text-center text-base lg:text-lg font-bold py-3 lg:py-3 rounded-xl lg:rounded-xl" />
                </div>
                <button disabled={!formData.name} onClick={nextStep} className="w-full max-w-xs mx-auto glass-button py-3 lg:py-3 mt-4 lg:mt-5 disabled:opacity-50 disabled:cursor-not-allowed text-sm lg:text-sm rounded-xl font-bold">Continue</button>
              </motion.div>
            )}

            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.3 }} className="space-y-3 lg:space-y-4 mt-2 lg:mt-3">
                <h2 className="text-xl lg:text-xl font-black mb-3 lg:mb-4 text-slate-800 text-center">Your Body Metrics</h2>
                <div className="grid grid-cols-2 gap-3 lg:gap-4 max-w-sm mx-auto">
                  <div>
                    <label className="text-[10px] lg:text-[10px] font-bold text-slate-400 tracking-widest uppercase mb-1 lg:mb-1 block text-center">Weight (kg)</label>
                    <input type="number" value={formData.weight} onChange={e => setFormData({...formData, weight: e.target.value})} className="w-full glass-input bg-slate-50 text-center text-sm lg:text-base font-bold py-2 lg:py-2 rounded-xl lg:rounded-xl" />
                  </div>
                  <div>
                    <label className="text-[10px] lg:text-[10px] font-bold text-slate-400 tracking-widest uppercase mb-1 lg:mb-1 block text-center">Height (cm)</label>
                    <input type="number" value={formData.height} onChange={e => setFormData({...formData, height: e.target.value})} className="w-full glass-input bg-slate-50 text-center text-sm lg:text-base font-bold py-2 lg:py-2 rounded-xl lg:rounded-xl" />
                  </div>
                  <div className="col-span-2">
                    <label className="text-[10px] lg:text-[10px] font-bold text-slate-400 tracking-widest uppercase mb-1 lg:mb-1 block text-center">Age</label>
                    <input type="number" value={formData.age} onChange={e => setFormData({...formData, age: e.target.value})} className="w-full glass-input bg-slate-50 text-center text-sm lg:text-base font-bold py-2 lg:py-2 rounded-xl lg:rounded-xl" />
                  </div>
                </div>
                <div className="flex justify-center pt-4 lg:pt-4 flex-col items-center gap-3 lg:gap-3">
                  {error && <p className="text-red-500 font-bold text-[11px] lg:text-xs bg-red-50 border border-red-100 px-3 lg:px-4 py-1.5 lg:py-1.5 rounded-xl w-full max-w-sm text-center animate-pulse">{error}</p>}
                  <button disabled={!formData.weight || !formData.height || !formData.age} onClick={() => {
                    ...
                    setError('');
                    finish();
                  }} className="w-full max-w-sm glass-button py-3 lg:py-3 text-sm lg:text-sm rounded-xl font-bold disabled:opacity-50 disabled:cursor-not-allowed">Generate My Plan</button>
                </div>
              </motion.div>
            )}
